...
exports.AbiToToolGenerator = void 0;
class AbiToToolGenerator {
    /**
        ...
        for (const func of functions) {
            ...
            const tool = {
                ...
                parameters: {
                    ...
                    required: func.inputs.map((input) => input.name)
                }
            };
    ...
    static generateFunctionDescription(contractName, func) {
    ...
    static inferFunctionPurpose(func) {
        const name = func.name.toLowerCase();
        if (name.includes('create'))
            return 'Creates a new entity or initiates a process.';
        if (name.includes('accept'))
            return 'Accepts or confirms a pending action.';
        if (name.includes('complete') || name.includes('finalize'))
            return 'Completes or finalizes an ongoing process.';
        if (name.includes('register'))
            return 'Registers a new entity in the system.';
        if (name.includes('update'))
            return 'Updates existing entity data.';
        if (name.includes('get') || name.includes('query'))
            return 'Retrieves information from the contract.';
        if (name.includes('conclude'))
            return 'Concludes a process and determines outcomes.';
        return 'Performs a contract operation.';
    }
    static generateParameterProperties(inputs) {
        const properties = {};
    ...
    static mapSolidityTypeToJsonSchema(solidityType) {
    ...
    static generateParameterDescription(input) {
        const name = input.name.toLowerCase();
        const type = input.type;
        if (type === 'address') {
            if (name.includes('owner'))
                return 'Ethereum address of the owner';
            if (name.includes('player') || name.includes('participant'))
                return 'Ethereum address of the player/participant';
            if (name.includes('token'))
                return 'Contract address of the ERC20 token';
            if (name.includes('controller'))
                return 'Ethereum address of the controller';
    ...
    static generatePreconditions(func) {
        const preconditions = [];
        const name = func.name.toLowerCase();
        // Common preconditions based on function patterns
        if (name.includes('create') && func.inputs.some((i) => i.name.includes('wager') || i.name.includes('reward'))) {
    ...
    static generateSideEffects(contractName, func) {
        const sideEffects = [];
        const name = func.name.toLowerCase();
        if (name.includes('create')) {
    ...
    static generateExampleParameters(func) {
        const example = {};
...
exports.AbiToToolGenerator = AbiToToolGenerator;
