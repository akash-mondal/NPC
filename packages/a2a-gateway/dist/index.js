...
const agentCard = new AgentCard_1.AgentCard(baseUrl);
...
async function handleTaskOpen(params) {
...
async function handleTaskStatus(params) {
    const { taskId } = params;
    if (!taskId) {
...
async function handleTaskUpdate(params) {
...
async function handleTaskFinalize(params) {
    const { taskId } = params;
    if (!taskId) {
        throw new Error('taskId is required');
    }
...
function broadcastToStreamClients(taskId, data) {
    const task = tasks.get(taskId);
    if (!task)
        return;
