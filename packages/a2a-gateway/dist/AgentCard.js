...
class AgentCard {
     ...
     */
    updateCapabilities(contractCapabilities) {
    ...
    validate() {
        ...
        for (const cap of this.data.capabilities) {
            if (!cap.name)
                errors.push(`Capability missing name: ${JSON.stringify(cap)}`);
            if (!cap.description)
                errors.push(`Capability ${cap.name} missing description`);
        }
        // Validate transport
        for (const transport of this.data.transport) {
            if (!transport.type)
                errors.push('Transport missing type');
            if (!transport.url)
                errors.push('Transport missing URL');
        }
        // Validate security schemes
        for (const security of this.data.security) {
            if (!security.type)
                errors.push('Security scheme missing type');
        }
        return {
     ...
     */
    static forNPC(npcId, baseUrl, contractAddresses) {
        ...
        card.data.contracts = contractAddresses;
        return card;
    }
}
